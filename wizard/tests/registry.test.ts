...
describe('MODULE_REGISTRY', () => {
  ...
  it('contains all required module entries', async () => {
    ...
    for (const moduleId of REQUIRED_MODULES) {
      expect(MODULE_REGISTRY, `MODULE_REGISTRY must contain '${moduleId}'`).toHaveProperty(
        moduleId
      );
    }
  });
  ...
  it('bmad and gsd entries are template-only (no npm devDeps)', async () => {
    const { MODULE_REGISTRY } = await import('../src/registry.js');
    // bmad and gsd are installed via template copy, not npm packages
    expect(MODULE_REGISTRY.bmad.devDeps).toHaveLength(0);
    expect(MODULE_REGISTRY.gsd.devDeps).toHaveLength(0);
  });

  it('no inline version strings — all versions come from dependency-versions.ts', async () => {
    const { MODULE_REGISTRY } = await import('../src/registry.js');
    const { HUSKY_VERSION, VITEST_VERSION } = await import('../src/dependency-versions.js');
